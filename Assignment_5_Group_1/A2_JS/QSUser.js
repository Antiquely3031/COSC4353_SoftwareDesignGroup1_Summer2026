...
// Port 5000: the A5 user backend (QSUserBackend/server.js), port 3000 belongs to the login backend
...
async function setupJoinQueuePage() {
    ...
    // integration of the smart wait time by replacing the call
    serviceSelect.addEventListener("change", async () => {
        updateSelectedService();
        await loadSmartWaitTime(serviceSelect.value);
    });
...
async function setupDashboardPage() {
    ...
    // Load available services from backend/database
    await loadDashboardServices();

    // Load current queue status from backend/database
    try {
        ...
        if (queuePosition) queuePosition.textContent = "--";
        if (waitTime) waitTime.textContent = "--";
        if (queueStatus) queueStatus.textContent = "Unable to load";
    }
}
//queue status page
