...
(function (global) {
    ...
    /* Which user these notifications belong to.
       ASSIGNMENT 4: the login screen stores the signed-in user under 'qs_user'
       as a JSON object, so we just read the id out of that. The older 'qs_userId'
       key is still checked so nothing breaks if a page sets it directly */
    function currentUserId() {
        if (!global.sessionStorage) { return '1'; }

        try {
            var stored = sessionStorage.getItem('qs_user');
            if (stored) {
                var parsed = JSON.parse(stored);
                if (parsed && parsed.id) { return String(parsed.id); }
            }
        } catch (e) {
            // bad session data, fall through to the other options
        }

        return sessionStorage.getItem('qs_userId') || '1';
    }
    ...
    /* ASSIGNMENT 4: moves this user's unread notifications from 'sent' to
       'viewed' in the database. */
    function markViewed() {
        return fetch(API_BASE + '/' + encodeURIComponent(USER_ID) + '/viewed', {
                method: 'PATCH'
            })
            .then(function (r) { return r.json(); })
            .catch(function () {
                // server unreachable, the rows just stay unread
                return null;
            });
    }
    ...
    // init
    function init() {
        // pull from the backend on load
        return refresh().then(function () {
            /* Only the full notifications page means the user actually read
               them. The dashboard shows a preview, so it must not mark
               anything as viewed. */
            if (document.getElementById('qs-notify-full')) {
                return markViewed();
            }
        });
    }
